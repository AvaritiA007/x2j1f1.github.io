...
class ViTEmbeddings(cnn.Module):
    def __init__(self, img_size=224, patch_size=16):
        super().__init__()
        self.projection = cnn.Conv2d(3, 768, kernel_size=patch_size, stride=patch_size)

    def forward(self, x):
        patches = self.projection(x)
        batch_size, channels, _, _ = patches.shape
        patches = patches.reshape(batch_size, channels, -1).permute(0, 2, 1)
        return patches


class ViTSelfAttention(cnn.Module):
    def __init__(self, embed_dim, num_heads):
        super().__init__()
        self.attn = cnn.MultiheadAttention(embed_dim, num_heads)
...
class ViTLayer(cnn.Module):
    def __init__(self, embed_dim, num_heads):
        super().__init__()
        self.attn = ViTSelfAttention(embed_dim, num_heads)
        self.mlp = nn.Sequential(
            cnn.Linear(embed_dim, embed_dim * 4),
            cnn.ReLU(),
            cnn.Linear(embed_dim * 4, embed_dim)
        )
...
class ViT(cnn.Module):
    def __init__(self, img_size=224, patch_size=16, num_classes=1000):
        super().__init__()
        self.patch_size = patch_size
        self.classifier = cnn.Linear(768, num_classes)
        self.embeddings = ViTEmbeddings(img_size, patch_size)
        self.layers = nn.ModuleList([ViTLayer(768, 12) for _ in range(12)])
